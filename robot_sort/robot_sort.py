class SortingRobot:
    ...
    def sort(self):
        ...
        while self.light_is_on():
            # if robot can't move right, and if the current item = None
            if self.can_move_right() == False and self.compare_item() == None:
                self.swap_item() # place last smallest item in correct place
                self.set_light_off()
                break # exit while loop post haste
            # grab smallest num in list
            while self.can_move_right():
                # if self.compare_item() == 1: # if held item is greater then current item in list
                #     self.swap_item()
                self.move_right()
                if self.compare_item() == 1: # if held item is greater then current item in list
                    self.swap_item()
            # put smallest num in correct position
            while self.can_move_left():
                if self.compare_item() == None: # if current item or held item is "NONE"
                    self.swap_item()
                    self.move_right()
                    self.swap_item()
                    break # exit this while loop post haste
                self.move_left()
                if self.compare_item() == None: # if current item or held item is "NONE"
                    self.swap_item()
                    self.move_right()
